<script lang="ts">
	...
	function setupSpotlightCards() {
		...
		// Add new listeners
		spotlightCards = Array.from(
			document.querySelectorAll(".spotlight-border"),
		) as HTMLElement[];
...
<svelte:head>
	...
	<meta
		name="description"
		content="Crafting digital experiences with focus and precision. Full Stack Developer with AWS & IBM certifications, building scalable applications."
	/>
	<meta
		name="keywords"
		content="full stack developer, web developer, react developer, aws certified, blockchain, Ghana developer"
	/>
...
<div
	class="fixed top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-blue-600 to-cyan-400 z-50 transition-all duration-150"
...
<div class="fixed inset-0 pointer-events-none overflow-hidden z-0">
	<div
		class="absolute top-1/4 left-1/4 w-[800px] h-[800px] bg-azure/20 rounded-full blur-[120px] animate-pulse-slow"
	></div>
	<div
		class="absolute bottom-1/4 right-1/4 w-[600px] h-[600px] bg-purple-500/10 rounded-full blur-[100px] animate-pulse-slower"
	></div>
</div>
...
>
	<div class="container mx-auto px-6 md:px-12 max-w-7xl">
		<div
			class="flex flex-col lg:grid lg:grid-cols-2 gap-12 md:gap-10 lg:gap-20 items-center min-h-[80vh] md:min-h-[600px]"
		>
			<!-- Left Side - Copy -->
			<div
				class="hero-content space-y-8 text-center md:text-center lg:text-left"
			>
				<!-- Status Indicator -->
				<div
					class="flex items-center gap-3 justify-center md:justify-center lg:justify-start"
				>
					<div class="status-dot"></div>
					<span class="text-text-body text-sm font-mono"
				...
				<h1
					class="text-4xl md:text-5xl lg:text-7xl font-bold leading-tight tracking-tight"
				>
					<span class="block text-text-heading mb-2"
						>Crafting digital</span
					>
					<span class="block text-text-heading mb-2"
						>experiences with</span
					>
					<span class="block">
						<span
							class="font-serif italic hero-gradient-text"
							style="animation-delay: 0s;">focus</span
						>
						<span class="text-text-heading"> and </span>
						<span
							class="font-serif italic hero-gradient-text"
							style="animation-delay: 0.5s;">precision</span
						>
					</span>
				</h1>
				...
				<!-- Subtext -->
				<p
					class="text-lg md:text-xl lg:text-2xl text-text-body max-w-2xl mx-auto lg:mx-0 leading-relaxed"
				>
					AWS Certified Full Stack Developer building scalable
					applications with modern technologies and innovative
					solutions.
				</p>
...
<section id="work" class="py-20 relative">
	<div class="container mx-auto px-6 md:px-8 max-w-7xl">
		...
		<div
			class="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6 reveal"
		>
			<h2
				class="text-4xl md:text-5xl font-bold text-text-heading tracking-tight"
			>
				Work & Credentials
			</h2>
			...
			<div class="flex items-center gap-4">
				<span
					class="text-text-body text-sm font-mono transition-colors {mode ===
					'builder'
						? 'text-text-heading font-bold'
						: 'text-text-muted'}">Builder</span
				>
				<button
					on:click={() =>
						(mode = mode === "builder" ? "thinker" : "builder")}
				...
				>
					<span
						class="absolute top-1 left-1 w-5 h-5 bg-azure rounded-full transition-transform duration-300 ease-out shadow-spotlight {mode ===
						'thinker'
							? 'translate-x-7'
							: ''}"
					></span>
  ...
  return unifiedResult;
			...
			<div
				class="md:col-span-2 lg:col-span-1 lg:row-span-2 h-auto bg-neutral-900/50 border border-white/10 rounded-xl p-4 overflow-hidden"
			>
				<h3 class="text-lg font-bold text-text-heading mb-3">
					Certifications
				</h3>
				<div
					class="flex flex-col md:flex-col gap-4 md:gap-2 overflow-y-auto md:overflow-y-auto h-auto md:h-full pb-2 md:pb-0 md:pr-2 scrollbar-hide"
				>
					{#each certifications as cert}
						...
						>
							<div
								class="w-12 h-12 rounded bg-zinc-800/50 flex items-center justify-center flex-shrink-0 overflow-hidden relative"
							>
								<img
									src={cert.image}
									alt={cert.title}
							...
							<div class="flex-1 min-w-0">
								<p
									class="text-text-heading text-xs font-semibold truncate"
								>
									{cert.title}
								</p>
								<p class="text-text-muted text-[10px] truncate">
									{cert.issuer}
								</p>
								<p class="text-azure text-[10px]">
									{cert.date}
								</p>
							</div>
						</a>
			...
			<div
				class="md:col-span-2 lg:col-span-1 lg:row-span-2 flex flex-col md:flex-row lg:flex-col gap-4"
			>
				<!-- Status Map (Top) -->
				<div
					class="flex-1 bg-neutral-900/50 border border-white/10 rounded-xl p-4 overflow-hidden relative group"
				>
					<div
						class="absolute inset-0 opacity-30 group-hover:opacity-50 transition-opacity"
					>
						<img
					...
					<div
						class="absolute top-3 left-3 z-20 bg-black/60 backdrop-blur-md border border-white/10 rounded-full px-3 py-1 flex items-center gap-2"
					>
						<div
							class="w-2 h-2 rounded-full bg-green-500 animate-pulse"
						></div>
						<span class="text-text-heading text-xs font-medium"
							>Open to Remote</span
						>
					</div>
					<div
					...
					>
						<div class="flex items-center gap-2 mb-2">
							<div class="status-dot"></div>
							<span
								class="text-text-heading font-semibold text-sm"
								>Status</span
							>
						</div>
						<p
							class="text-text-body text-xs font-mono font-semibold"
						>
							Tema, Ghana
						</p>
				...
				<div
					class="flex-[1.2] bg-neutral-900/50 border border-white/10 rounded-xl p-4 overflow-hidden relative group"
				>
					<!-- Blurred Album Art Background -->
					<div
						class="absolute inset-0 bg-gradient-to-br from-purple-600/30 via-pink-600/30 to-purple-800/30 blur-xl"
					></div>
					<div
						class="relative z-10 h-full flex flex-col justify-between md:justify-start lg:justify-between"
					>
						<div>
							<div class="flex items-center gap-2 mb-2">
								<svg
							...
							<p class="text-text-body text-xs font-mono mb-3">
								Focus Playlist
							</p>
							<!-- Waveform Animation -->
							<div
								class="flex items-end gap-1 h-8 mb-3 md:mb-0 lg:mb-3"
							>
								<div
									class="w-1 bg-green-500/60 rounded-full waveform-bar"
									style="height: 20%; animation-delay: 0s;"
								></div>
								<div
									class="w-1 bg-green-500/60 rounded-full waveform-bar"
									style="height: 60%; animation-delay: 0.1s;"
								></div>
								<div
									class="w-1 bg-green-500/60 rounded-full waveform-bar"
									style="height: 100%; animation-delay: 0.2s;"
								></div>
								<div
									class="w-1 bg-green-500/60 rounded-full waveform-bar"
								...
								<div
									class="w-1 bg-green-500/60 rounded-full waveform-bar"
									style="height: 80%; animation-delay: 0.4s;"
								></div>
							</div>
						</div>
						...
						<div
							class="playlist-marquee-container relative h-24 overflow-hidden md:mt-2 md:h-40"
						>
							<!-- Gradient Masks -->
							<div
								class="absolute top-0 left-0 right-0 h-6 bg-gradient-to-b from-purple-600/30 to-transparent z-10 pointer-events-none"
							></div>
							<div
								class="absolute bottom-0 left-0 right-0 h-6 bg-gradient-to-t from-purple-600/30 to-transparent z-10 pointer-events-none"
							></div>

							<!-- Scrolling Song List -->
							<div class="playlist-marquee space-y-1.5">
								<!-- First set -->
								{#each [{ title: "Frens", artist: "Obongjayar" }, { title: "History", artist: "Dave, James Blake" }, { title: "For You", artist: "Kadjha Bonet" }, { title: "I wish it was me", artist: "Obongjayar" }, { title: "Queen Tings", artist: "Masego, Tiffany Gouché" }, { title: "Free Mind", artist: "Tems" }, { title: "Chapter 16", artist: "Dave, Kano" }, { title: "First in Class", artist: "Omar Sterling" }, { title: "We dey manage", artist: "M.anifest" }, { title: "Wonderful Wonder", artist: "Nathaniel Bassey, Love Song" }, { title: "Disco", artist: "Surf Curse" }] as song}
									<div class="playlist-item">
										<div class="flex items-start gap-2">
											<div
												class="w-1.5 h-1.5 rounded-full bg-green-500 flex-shrink-0 mt-0.5"
											></div>
											<div class="flex-1 min-w-0">
												<p
													class="text-text-heading text-[10px] font-medium truncate leading-tight"
												>
													{song.title}
												</p>
												<p
													class="text-text-muted text-[9px] truncate leading-tight"
												>
													{song.artist}
												</p>
											</div>
			...
			<div
				class="md:col-span-2 lg:col-span-2 lg:row-span-1 h-auto bg-neutral-900/50 border border-white/10 rounded-xl p-4 overflow-hidden"
			>
				{#key mode}
					{#if mode === "thinker"}
						...
						>
							...
							simplifying crypto payments or guiding someone
							through their daily challenges, I build software
							that solves real human problems. At the heart of it
							all, it's never about money or fame; it's about
							using technology to make life better for others and
					...
					{:else}
						...
						<div class="flex items-center justify-between mb-3">
							...
							<span
								class="pill-badge bg-azure/20 text-azure border-azure/50 animate-pulse text-xs px-2 py-0.5"
								>Active</span
							>
						</div>
						<div class="space-y-3">
							...
							<div class="tech-marquee-container overflow-hidden">
								<div class="tech-marquee-left flex gap-6">
									<!-- First set -->
									{#each [{ name: "React", icon: "/images/tech/react.svg" }, { name: "TypeScript", icon: "/images/tech/typescript.svg" }, { name: "Node.js", icon: "/images/tech/nodejs.svg" }, { name: "SvelteKit", icon: "/images/tech/svelte.svg" }] as tech}
										<div
											class="flex items-center gap-2 flex-shrink-0 tech-item"
										>
											<div
												class="w-8 h-8 rounded-lg bg-zinc-800/50 flex items-center justify-center"
											>
												<img
													src={tech.icon}
													alt={tech.name}
											...
											<span
												class="text-text-body text-sm font-mono"
												>{tech.name}</span
											>
										</div>
									{/each}
							...
							<div class="tech-marquee-container overflow-hidden">
								<div class="tech-marquee-right flex gap-6">
									<!-- First set -->
									{#each [{ name: "Supabase", icon: "/images/tech/supabase.svg" }, { name: "PostgreSQL", icon: "/images/tech/postgresql.svg" }, { name: "Firebase", icon: "/images/tech/firebase.svg" }, { name: "Tailwind", icon: "/images/tech/tailwindcss.svg" }, { name: "AWS", icon: "/images/tech/aws.svg" }] as tech}
										<div
											class="flex items-center gap-2 flex-shrink-0 tech-item"
										>
											<div
			...
			<div
				class="md:col-span-1 lg:col-span-1 lg:row-span-1 bg-neutral-900/50 border border-white/10 rounded-xl p-4 overflow-hidden"
			>
			...
			<div
				class="md:col-span-1 lg:col-span-1 lg:row-span-1 min-h-[180px] md:min-h-0 bg-neutral-900/50 border border-white/10 rounded-xl p-4 overflow-hidden relative group"
			>
					...
					>
						<defs>
							...
							>
								<stop
									offset="0%"
									style="stop-color:rgb(16,185,129);stop-opacity:0.3"
								/>
								<stop
									offset="100%"
									style="stop-color:rgb(6,182,212);stop-opacity:0.3"
								/>
							</linearGradient>
						</defs>
						<path
							d="M 0,80 Q 20,70 30,60 T 50,40 T 70,25 T 100,10"
						...
						<circle
							...
							cy="10"
							r="2"
							fill="url(#carousel-trend-gradient)"
						/>
					</svg>
				</div>
				<div class="relative z-10 h-full flex flex-col justify-between">
					<div class="flex items-start justify-between mb-2">
						<span class="text-text-heading font-semibold text-sm"
							>Projects</span
						>
						<div class="flex gap-1">
							{#each carouselSlides as _, index}
								<div
									class="w-1 h-1 rounded-full {index ===
									currentSlide
										? 'bg-white'
					...
					<div class="carousel-container relative flex-1">
						{#each carouselSlides as slide, index}
							<div
								class="carousel-slide absolute inset-0 {index ===
								currentSlide
									? 'opacity-100'
							...
							>
								<div class="flex items-center gap-1.5 mb-2">
									<div
										class="w-2 h-2 rounded-full {slide.statusColor}"
									></div>
									<span
										class="text-text-muted text-[9px] font-mono"
										>{slide.status}</span
									>
								</div>
								<h4
									class="text-text-heading font-semibold text-xs mb-1.5"
								>
									{slide.title}
								</h4>
								...
								<p
									class="text-text-muted text-[9px] font-mono leading-tight"
								>
									{slide.subtext}
								</p>
							</div>
...
<section id="experience" class="pt-20 pb-24 relative -mt-8">
	<div class="container mx-auto px-6 md:px-8 max-w-4xl">
		<h2
			class="text-4xl md:text-5xl font-bold text-center mb-16 text-text-heading tracking-tight reveal"
		>
			Experience
		</h2>
		...
		<div class="relative">
			...
			<div
				class="absolute left-0 md:left-[15%] top-0 bottom-0 w-0.5 bg-gradient-to-b from-blue-500 via-purple-500 to-transparent opacity-60"
			></div>

			<div class="space-y-12 pl-12 md:pl-[20%]">
				{#each experiences as experience, index (experience.id)}
					{@const isLeadership =
						experience.position === "Technical Project Lead"}
					<div class="relative reveal experience-item group">
						<!-- Timeline Dot - Hollow by default, fills on hover -->
						<div
							class="absolute left-0 md:left-[-15%] top-6 w-4 h-4 rounded-full bg-obsidian-base border-2 border-blue-500 transform -translate-x-1/2 z-10 transition-all duration-300 group-hover:bg-blue-500 group-hover:border-blue-400 group-hover:shadow-[0_0_12px_rgba(59,130,246,0.6)] {isLeadership
								? 'border-amber-500 group-hover:bg-amber-500 group-hover:border-amber-400 group-hover:shadow-[0_0_12px_rgba(245,158,11,0.6)]'
								: ''}"
						></div>

						<!-- Content Card -->
						<div
							class="glass-card spotlight-border rounded-xl p-6 transition-all duration-300 group-hover:scale-[1.02] group-hover:border-white/20 {isLeadership
						...
						>
							...
							<div class="mb-3">
								<span
									class="text-xs font-mono text-zinc-400 tracking-wider"
								>
							...
							<!-- Role - Larger, Bold -->
							<h3
								class="text-xl font-bold text-text-heading mb-2"
							>
								{experience.position}
							</h3>
							...
							<ul class="space-y-2 mt-4">
								{#each experience.description as item}
									<li
										class="text-text-body text-sm flex items-start"
									>
										<span
											class="text-azure mr-2 flex-shrink-0"
											>•</span
										>
...
<section id="contact" class="py-20 relative overflow-hidden">
	...
	<div
		class="absolute bottom-0 left-0 right-0 h-96 bg-gradient-to-t from-blue-900/20 to-transparent pointer-events-none"
	></div>
	...
	<div class="container mx-auto px-6 md:px-8 max-w-7xl relative z-10">
		...
		<div class="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
			...
			<div class="reveal">
				<h2
					class="text-5xl md:text-6xl font-bold mb-6 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-600 drop-shadow-lg"
				>
					Ready to Ship?
				</h2>
				<p class="text-text-body text-lg mb-8 leading-relaxed">
					I'm currently open for new projects. Whether you have a
				...
				<div class="mb-8">
					<div
						class="block text-sm font-mono text-text-muted mb-2 uppercase tracking-wider"
					>
						Direct Line
					</div>
					<button
					...
					>
						<span class="text-text-heading font-mono text-lg"
							>danielmawuena75@gmail.com</span
						>
						<svg
							class="w-5 h-5 text-text-muted group-hover:text-azure transition-colors"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
					...
					{#if emailCopied}
						<div
							class="mt-2 text-green-400 text-sm font-mono animate-fade-in"
						>
							✓ Copied to clipboard!
						</div>
					{/if}
				...
				<div class="space-y-4">
					<div
						class="block text-sm font-mono text-text-muted mb-3 uppercase tracking-wider"
					>
						Connect
					</div>
					<a
					...
					>
						...
						>
						<svg
							class="w-4 h-4 text-text-muted ml-auto group-hover:translate-x-1 transition-transform"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
					...
					>
						<svg
							class="w-6 h-6 text-text-heading"
							fill="currentColor"
							viewBox="0 0 24 24"
						>
							<path
			...
			<div class="reveal">
				<div
					class="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-8"
				>
					<form on:submit={handleSubmit} class="space-y-6">
						...
						{#if status === "success"}
							<div
								class="p-4 bg-green-900/30 border border-green-500 rounded-lg text-green-400 font-mono text-sm"
							>
								{feedback ||
						...
						{:else if status === "error"}
							<div
								class="p-4 bg-red-900/30 border border-red-500 rounded-lg text-red-400 font-mono text-sm"
							>
								{feedback ||
...
{#if showBackToTop}
	...
	>
		<svg
			class="w-6 h-6 text-white transform group-hover:-translate-y-1 transition-transform"
			fill="none"
			stroke="currentColor"
			viewBox="0 0 24 24"
		>
...
<style>
	...
	.leadership-circuit-card:hover {
		...
		box-shadow:
			...
			0 0 80px -10px rgba(245, 158, 11, 0.3);
	}
